import requests
from ozkokapitest.src.configs.hosts_config import API_HOSTS
...
class RequestUtility(object):
    ...
    def post(self, endpoint, payload=None, headers=None, expected_status_code=200):
        ...
        rs_api = requests.post(self.url, data=json.dumps(
            payload), headers=headers, auth=self.auth)
    ...
    def get(self, endpoint, payload=None, headers=None, expected_status_code=200):
        ...
        self.url = self.base_url + endpoint
        rs_api = requests.get(self.url, data=json.dumps(
            payload), headers=headers, auth=self.auth)
        self.status_code = rs_api.status_code
        self.expected_status_code = expected_status_code
        self.rs_json = rs_api.json()
